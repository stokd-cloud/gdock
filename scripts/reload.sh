...
derive_socket_marker_names() {
  ...
  case "$bundle_id" in
    # gdock flavors. Upstream replaced the fork's own write_last_socket_path
    # with this table; without these entries every gdock bundle id falls to the
    # default branch and nightly/staging/dev builds all clobber the single
    # unscoped marker. Marker names match what the fork wrote before the sync.
    ...
    cloud.stokd.ghostty-dock.nightly)
      ...
      ;;
    cloud.stokd.ghostty-dock.nightly.*)
      variant_slug="$(sanitize_path "${bundle_id#cloud.stokd.ghostty-dock.nightly.}")"
    ...
    cloud.stokd.ghostty-dock.staging)
      ...
      ;;
    cloud.stokd.ghostty-dock.staging.*)
      variant_slug="$(sanitize_path "${bundle_id#cloud.stokd.ghostty-dock.staging.}")"
      if [[ -n "$variant_slug" ]]; then
    ...
    cloud.stokd.ghostty-dock.debug)
      ...
      ;;
    cloud.stokd.ghostty-dock.debug.*)
      variant_slug="$(sanitize_path "${bundle_id#cloud.stokd.ghostty-dock.debug.}")"
      if [[ -n "$variant_slug" ]]; then
        CMUX_RELOAD_MARKER_NAME="dev-${variant_slug}-last-socket-path"
        CMUX_RELOAD_TMP_MARKER="/tmp/cmux-dev-${variant_slug}-last-socket-path"
      else
...
bundle_socket_path() {
  ...
  local app_name="\${bundle_path##*/}"
  app_name="\${app_name%.app}"
  # gdock renames tagged apps ("gdock <tag>", "gdock DBG <tag>", "gdock DEV
  # <tag>"); upstream only knows "cmux DEV <tag>". Without these prefixes
  # live_cli_bundle can never resolve a socket for a fork-built tagged app, so
  # the tag-aware CLI redirect below would silently fall through.
  local tag=""
  case "\$app_name" in
    "cmux DEV "*)  tag="\${app_name#cmux DEV }" ;;
    "gdock DBG "*) tag="\${app_name#gdock DBG }" ;;
    "gdock DEV "*) tag="\${app_name#gdock DEV }" ;;
    "gdock "*)     tag="\${app_name#gdock }" ;;
    *) return 1 ;;
  esac
  ...
  if [[ "\$SOCKET_NAME" == cmux-debug-*.sock ]]; then
    ...
    if [[ "\$TAG" =~ ^[A-Za-z0-9_-]+$ ]]; then
      ...
      for TAG_CONFIG in Release Debug; do
        for TAG_APP in "gdock \$TAG" "gdock DBG \$TAG" "gdock DEV \$TAG" "cmux DEV \$TAG"; do
      ...
      # Upstream's liveness gate: only redirect to the tagged CLI when its app
      # bundle is actually running, so a stale build does not swallow commands.
      if [[ -n "\$BEST_TAG_CLI" ]] && live_cli_bundle "\$BEST_TAG_CLI" >/dev/null; then
