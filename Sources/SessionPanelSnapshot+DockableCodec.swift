...
extension SessionPanelSnapshot {
    ...
    func encodeDockPayloadData() throws -> Data {
        ...
        case .simulator:
            ...
            return try encoder.encode(simulator)
        case .notifications:
            // Marker payload; the feed itself lives in the notification store.
            return try encoder.encode(notificationsPanel ?? SessionNotificationsPanelSnapshot())
        case .extensionBrowser, .cloudVMLoading, .mobilePairing, .accountSignIn:
    ...
    var hasRestorableTypedPayload: Bool {
        ...
        case .simulator: return simulator != nil
        case .notifications: return true
        case .extensionBrowser, .cloudVMLoading, .mobilePairing, .accountSignIn: return false
        }
    }
