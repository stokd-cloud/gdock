...
struct MacBuildChannelTests {
    @Test func devTagWinsAndIsShown() {
        ...
        #expect(MacBuildChannel().label(bundleID: "cloud.stokd.ghostty-dock.debug.teams", tag: "teams") == "DEV · teams")
        #expect(MacBuildChannel().label(bundleID: "cloud.stokd.ghostty-dock", tag: "my-tag") == "DEV · my-tag")
    }

    @Test func channelFromBundleComponentWhenNoDevTag() {
        #expect(MacBuildChannel().label(bundleID: "cloud.stokd.ghostty-dock", tag: "default") == "Stable")
        #expect(MacBuildChannel().label(bundleID: "cloud.stokd.ghostty-dock.nightly", tag: "default") == "Nightly")
        // Tagged channel builds append a further .slug — match the COMPONENT, not a suffix.
        #expect(MacBuildChannel().label(bundleID: "cloud.stokd.ghostty-dock.nightly.my-feature", tag: "default") == "Nightly")
        #expect(MacBuildChannel().label(bundleID: "cloud.stokd.ghostty-dock.staging.feat", tag: nil) == "Staging")
        #expect(MacBuildChannel().label(bundleID: "cloud.stokd.ghostty-dock.debug", tag: "default") == "DEV")
    }

    @Test func handlesFutureReleaseCandidateChannel() {
        // The RC desktop build (cloud.stokd.ghostty-dock.rc) is handled ahead of time.
        #expect(MacBuildChannel().label(bundleID: "cloud.stokd.ghostty-dock.rc", tag: "default") == "RC")
        #expect(MacBuildChannel().label(bundleID: "cloud.stokd.ghostty-dock.rc.candidate1", tag: nil) == "RC")
    }
    ...
    @Test func nilWhenNotIdentifiable() {
        ...
        #expect(MacBuildChannel().label(bundleID: "cloud.stokd.ghostty-dock.beta", tag: "default") == nil)
    }
}
