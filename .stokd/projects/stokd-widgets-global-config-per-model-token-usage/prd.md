# Stokd Widgets: Global Config + Per-Model Token Usage

## 0. Source Context

**Derived From:** "port two of stokd code's stokd extension panels — the global configs and the one that tracks token usage per agent — to mac widgets. Token usage should parse `.stokd/config.yaml`'s configured providers and break them down by models like the ones listed on `stokd model list` (which the existing extension version doesn't do well). Prefer a watch over a poll, defer to the most resource-efficient implementation. Global configs must leverage the cwd of whatever the active window is to provide the context to show the right things in all cases."

**Feature Name:** Stokd Widgets: Global Config + Per-Model Token Usage
**PRD Owner:** Brian Stoker
**Last Updated:** 2026-07-29
**Repo:** `stokd-cloud/ghostty-dock` (fork of `manaflow-ai/cmux`; internal product name still `cmux`, fork product name **gdock**)
**Companion PRD:** `stokd-cloud/mono` → `docs/port-stokd-panels-to-ghostty-dock.prd.md` ports eight *other* stokd panels (worktrees, work, reviews, model config, workload config, activity, agents, ACP chat). It does **not** cover either panel in this document. The two PRDs overlap only in that both need a stokd data plane; this one deliberately ships its own narrow package rather than blocking on that PRD's `StokdKit`.

### Summary

Port two panels out of the stokd VS Code extension into gdock as **widgets**: compact, glanceable, singleton dockable canvas panes styled as rounded tiles (the macOS Notification Center idiom — a weather tile, a calendar tile), not full-height sidebar views.

Widget 1 — **Global Config**: a data-driven settings form whose fields come from `stokd config schema --json` (74 descriptors across 23 groups), showing per-layer provenance (default / global / workspace / effective) and writing exclusively through the canonical CLI writer. Its scope context is resolved from the **cwd of the active window's focused workspace**, so the same widget shows the right workspace overlay as the user moves between windows.

Widget 2 — **Token Usage**: per-**provider** → per-**model** token and cost breakdown across four timespans. Providers come from `.stokd/config.yaml`; the model universe comes from `stokd model list --json`. Data is ingested by **watching provider transcript files and incrementally tailing only what changed** — never by polling an aggregate endpoint on a timer, which is exactly what the existing extension does (`REFRESH_INTERVAL_MS = 60_000`) and what makes its per-model breakdown thin.
...
- Ship a reusable **widget tile chrome** (`WidgetTile`) so glanceable stokd surfaces share one compact visual language, distinct from full-size panels.
- Ship the **Global Config widget**: 74 schema-driven fields, grouped, searchable, with per-layer provenance badges and inline revert.
- Resolve config scope from the **active window's focused-workspace cwd**, live, with no user action — including the "no workspace config exists yet" case.
- Ship the **Token Usage widget**: provider → model → {input, output, cache-read, cache-write, cost} across `24h` / `week` / `month` / `total`.
- Ingest token data **watch-first**: FSEvents on provider transcript roots, byte-offset incremental tail, coalesced aggregation. No fixed-interval polling of a REST aggregate.
- Attribute every token to a **model id** and a **provider**, and make unattributable usage visibly `unknown` rather than silently dropped or mispriced.
- Make cost honesty explicit: a model with no price entry renders tokens with an `unpriced` marker, never a fabricated dollar figure.
- Keep all config writes routed through the canonical CLI writer so routing, validators, and secret protection always run.

### Constraints

- **Prerequisite gate.** The canvas Dockable refactor is **not** in `main`. Verified: `DockableKind`, `Dockable`, `DockableSnapshot`, and `Packages/macOS/CmuxDockable` do not exist in `main`; a case-insensitive grep for `dockable` across `Sources/`, `Packages/`, and `cmuxTests/` returns zero matches. They exist only on the unlanded branch `mission/canvas-dockable-refactor-20260725-170934` (2 commits atop `6e788a05f3`, 66 files, +6240/−228) in worktree `/opt/worktrees/stokd-cloud/ghostty-dock/canvas-dockable-mission-20260725-170934`. **Phase 1 gates on that branch landing** and this PRD targets the post-refactor shape (`Dockable` / `DockableKind` / `DockableSnapshot`), because "side controls as dockable canvas panes" (`docs/gdock-side-controls-iteration.md`) is precisely the singleton-canvas-pane pattern both widgets need, and authoring against the legacy `PanelType` shape would mean editing 8 exhaustive switch sites and then immediately rewriting them.
- **Not WidgetKit.** These are in-app dockable panes styled as widget tiles, not a WidgetKit extension. WidgetKit runs in a separate process on an OS-throttled timeline budget and cannot observe the host app's active-window cwd or sustain a live file watch — both of which are explicit requirements here. See §5 Decision D1.
- **No Mongo change streams.** The local `mongod` is standalone (`{hello:1}` → `setName: (none)`); change streams require a replica set. Mongo is a cold-start backfill source only.
- **The realtime gateway cannot carry this data.** `telemetry.cost_update` (`packages/api/src/modules/telemetry/telemetry.service.ts:2181-2188`) emits only `{inputTokens, outputTokens, costUsd, cumulativeCostUsd}` — **no `model`, no cache dimensions**. It is usable as a nudge-to-refresh hint, never as the per-model source. Its transport is Redis pub/sub and no-ops when `REDIS_URL` is unset.
- **Mongo coverage is unfit as the primary source.** `telemetry_sessions` holds 555 docs of which only 25 have non-empty `model_usage` and 11 have `input_tokens > 0` (4.5%), and TTLs stopped sessions 7 days after `ended_at`. `session_telemetry` has no TTL but holds 45 bedrock-only rows and uses different field names (`input`/`output`/`cache_read`/`cache_write`/`reasoning`) with percent-escaped model keys.
- **Config writes go through the CLI only** (`AX-CLI-CONFIG-WRITES-THROUGH-CANONICAL-WRITER`). The widget never mutates YAML directly. The schema is sourced from the CLI descriptor (`AX-CLI-CONFIG-SCHEMA-DESCRIPTOR`), never hardcoded.
- **`~/.stokd/config.yaml` is the user's global config and is agent-off-limits.** A repo hook blocks agent writes to it. The widget's *default* write scope is therefore `--workspace`; global writes require an explicit, deliberate scope switch in the UI.
- **Fork/upstream split (Model B).** `origin/upstream-main` is a pristine upstream mirror kept current by `.github/workflows/sync-upstream.yml`; fork work lands on `origin/main`. There is no `upstream` git remote configured (only `origin` → `git@github.com:stokd-cloud/ghostty-dock.git`). Stokd-specific code is fork-only by nature; exactly one work item (2.2) is generic enough to upstream.
- **Localization.** `Resources/Localizable.xcstrings` carries **20** locales (`ar bs da de en es fr it ja km ko nb pl pt-BR ru th tr uk zh-Hans zh-Hant`), not the two `CLAUDE.md` claims. There is **no** localization CI gate; enforcement is by review. Strings use stock Foundation `String(localized:defaultValue:)` — there is no cmux shim.
- **Typing-latency and SwiftUI list rules apply.** Per `CLAUDE.md`: no store references below a `LazyVStack`/`List`/`ForEach` boundary (rows take value snapshots + closure action bundles); no `@Published` writes from inside a view-body computation.
- **`CMUX_SKIP_ZIG_BUILD=1` is required** on local builds in this environment — host zig is 0.16.0 while the ghostty submodule pins 0.15.2, so a bare build fails in the CLI-helper step.
...
|------|-------------|-----------------|----------------|
| Xcode | 16.0 | Apple Developer downloads | `xcodebuild -version` |
| Swift | 6.0 | bundled with Xcode | `swift --version` |
| Python | 3.11 | `brew install python@3.11` | `python3 --version` |
| stokd CLI | 0.2.53 | `cargo install --path apps/cli` (from `stokd-cloud/mono`) | `stokd --version` |
| git | 2.40 | `brew install git` | `git --version` |
| gh | 2.40 | `brew install gh` | `gh --version` |

No new third-party Swift dependency is introduced. All watching uses `CmuxFoundation`'s existing `FileWatcher` / `RecursivePathWatcher`; all JSON uses `Foundation.JSONDecoder`; SQLite reads use the system `libsqlite3` already linked by the app.
...
**Purpose:** Must come first because every later phase imports types from a package that does not exist yet, and because the Dockable refactor this PRD targets is not in `main` — building panels against the legacy `PanelType` shape would produce work that Phase 2 immediately discards. This phase establishes the gate and the dependency-free leaf types (schema decoding, CLI invocation) that Phases 2 and 3 both build on.
...
- Test-1.1.a: Regression — read `Package.swift`; assert single product + single test target (satisfies AC-1.1.a).
- Test-1.1.b: Integration — the gate command in AC-1.1.b run against `main`.
- Test-1.1.c: Integration — `swift build` of the new package (AC-1.1.c).
- Test-1.1.d: Regression — workspace-group checker (AC-1.1.d), guards the drift class CI already enforces.
- Test-1.1.e: Regression — grep the CI workflow (AC-1.1.e).
- Test-1.1.f: Regression — lockfile policy checker (AC-1.1.f).

**Verification Commands**

```bash
set -euo pipefail
test -f Packages/macOS/CmuxDockable/Sources/CmuxDockable/DockableKind.swift
test -f Packages/macOS/StokdWidgetKit/Package.swift
swift build --package-path Packages/macOS/StokdWidgetKit
python3 scripts/check-workspace-package-groups.py --check
python3 scripts/check-package-resolved-policy.py
grep -q 'StokdWidgetKit' .github/workflows/ci.yml
```
...
- Test-1.2.a: Unit — `StokdConfigDescriptorTests.boolAndBooleanDecodeToSameCase` (AC-1.2.a).
- Test-1.2.b: Unit — `StokdConfigDescriptorTests.decodesCheckedInLiveFixtureWithoutError`, run against a committed fixture captured from `stokd config schema --json` (AC-1.2.b).
- Test-1.2.c: Unit — `StokdLayeredValueTests.precedenceWorkspaceOverGlobalOverDefault` (AC-1.2.c).
- Test-1.2.d: Unit — `StokdLayeredValueTests.serializeForCliArrayAndBool` (AC-1.2.d).
- Test-1.2.e: Unit — the whole suite (AC-1.2.e).
- Test-1.2.f: Integration — live CLI contract probe (AC-1.2.f).

**Verification Commands**

```bash
set -euo pipefail
test -f Packages/macOS/StokdWidgetKit/Sources/StokdWidgetKit/Config/StokdConfigFieldDescriptor.swift
test -f Packages/macOS/StokdWidgetKit/Sources/StokdWidgetKit/Config/StokdLayeredValue.swift
swift test --package-path Packages/macOS/StokdWidgetKit --filter StokdConfigDescriptorTests
swift test --package-path Packages/macOS/StokdWidgetKit --filter StokdLayeredValueTests
stokd config schema --json | python3 -c 'import json,sys; d=json.load(sys.stdin); assert isinstance(d,list) and d; ks={k for f in d for k in f}; assert ks <= {"key","type","default","enumValues","group","label","description","scope","secret","localOnly"}, ks; print("descriptor fields:", len(d))'
```
...
- Test-1.3.a: Unit — `StokdCLIRunnerTests.passesWorkingDirectoryToChildProcess`, asserted with a `/bin/pwd`-style probe (AC-1.3.a).
- Test-1.3.b: Unit — `StokdCLIRunnerTests.missingExecutableReturns127` (AC-1.3.b).
- Test-1.3.c: Unit — `StokdWorkspaceConfigLocatorTests.matchesRustWalkAcrossFourFixtures`, using temp-dir fixtures mirroring the Rust tests at `apps/cli/src/config.rs:7773-7804` (AC-1.3.c).
- Test-1.3.d: Security — `StokdConfigWriterTests.allMutationsRouteThroughCLI`, asserting the injected runner recorded every mutation and no filesystem writer was constructed (AC-1.3.d).
- Test-1.3.e: Unit — the runner suite (AC-1.3.e).
- Test-1.3.f: Unit — the locator suite (AC-1.3.f).
- Test-1.3.g: Security — the static grep guard (AC-1.3.g), a regression fence for `AX-CLI-CONFIG-WRITES-THROUGH-CANONICAL-WRITER`.

**Verification Commands**

```bash
set -euo pipefail
swift test --package-path Packages/macOS/StokdWidgetKit --filter StokdCLIRunnerTests
swift test --package-path Packages/macOS/StokdWidgetKit --filter StokdWorkspaceConfigLocatorTests
swift test --package-path Packages/macOS/StokdWidgetKit --filter StokdConfigWriterTests
if grep -rn 'config\.yaml' Packages/macOS/StokdWidgetKit/Sources --include=*.swift | grep -qE '\.write|createFile'; then
  echo "FAIL: direct config.yaml write found in StokdWidgetKit" >&2; exit 1
fi
```
...
**Purpose:** Cannot start before Phase 1 because every item here imports `StokdWidgetKit` types (`StokdCLIRunner` for the model registry, the config value enum for provider parsing) from a package Phase 1 creates. This phase is the diamond middle of the dependency graph: the ingest engine (2.1–2.4) and the widget host (2.5–2.6) are mutually independent but both strictly precede Phase 3, which needs both a data source and a mounted pane to render into. They are one phase precisely because they may proceed in either order.
...
- Test-2.1.a: Unit — `TranscriptDecoderTests.claudeMapsAllFourDimensions` (AC-2.1.a).
- Test-2.1.b: Regression — `TranscriptDecoderTests.claudeIgnoresIterationsAndDoesNotDoubleCount` (AC-2.1.b). This is the specific arithmetic error that would silently double every Claude number.
- Test-2.1.c: Unit — `TranscriptDecoderTests.codexLatchesModelAcrossTokenCountLines` (AC-2.1.c).
- Test-2.1.d: Unit — `TranscriptDecoderTests.codexUnattributedBecomesUnknownNotDropped` (AC-2.1.d).
- Test-2.1.e: Regression — `TranscriptDecoderTests.truncatedTrailingLineIsNotConsumed` (AC-2.1.e).
- Test-2.1.f: Unit — the whole decoder suite (AC-2.1.f).

**Verification Commands**

```bash
set -euo pipefail
swift test --package-path Packages/macOS/StokdWidgetKit --filter TranscriptDecoderTests
test -d Packages/macOS/StokdWidgetKit/Tests/StokdWidgetKitTests/Fixtures
ls Packages/macOS/StokdWidgetKit/Tests/StokdWidgetKitTests/Fixtures | grep -q 'claude'
ls Packages/macOS/StokdWidgetKit/Tests/StokdWidgetKitTests/Fixtures | grep -q 'codex'
```
...
- Test-2.2.a: Performance — `JSONLTailReaderTests.readsOnlyAppendedRegion`, asserting the byte count read (AC-2.2.a). A whole-file re-read would pass a naive correctness test, so the assertion is on bytes, not content.
- Test-2.2.b: Regression — `JSONLTailReaderTests.partialLineDeliveredExactlyOnce` (AC-2.2.b).
- Test-2.2.c: Unit — `JSONLTailReaderTests.truncationRestartsFromZero` (AC-2.2.c).
- Test-2.2.d: Unit — `JSONLTailReaderTests.unchangedFileYieldsNothing` (AC-2.2.d).
- Test-2.2.e: Unit — the whole suite (AC-2.2.e).
- Test-2.2.f: Regression — the stokd-free grep (AC-2.2.f), guarding the upstream/fork boundary.

**Verification Commands**

```bash
set -euo pipefail
test -f Packages/macOS/CmuxFoundation/Sources/CmuxFoundation/FileWatch/JSONLTailReader.swift
swift test --package-path Packages/macOS/CmuxFoundation --filter JSONLTailReaderTests
if grep -riq 'stokd' Packages/macOS/CmuxFoundation/Sources/CmuxFoundation/FileWatch/JSONLTailReader.swift; then
  echo "FAIL: upstream-bound file references stokd" >&2; exit 1
fi
```
...
- Test-2.3.a: Performance — `TokenUsageIngestorTests.opensOnlyChangedFiles` (AC-2.3.a).
- Test-2.3.b: Regression — `TokenUsageIngestorTests.nilWatcherInitYieldsNoWatchableRootsState` (AC-2.3.b).
- Test-2.3.c: Unit — `TokenUsageIngestorTests.coalescesBurstIntoSingleSnapshot` using the injectable `FileWatchClock` (AC-2.3.c).
- Test-2.3.d: Unit — `TokenUsageIngestorTests.timespanBucketArithmeticAndLifetimeTotal` (AC-2.3.d).
- Test-2.3.e: Performance — `TokenUsageIngestorTests.prunesToWindowWithoutLosingLifetime` (AC-2.3.e).
- Test-2.3.f: Regression — `TokenUsageIngestorTests.lifetimeTotalSurvivesRestart` (AC-2.3.f). Without this, `total` silently resets on every launch.
- Test-2.3.g: Performance — `TokenUsageIngestorTests.unreachableAPIColdStartRespectsBackfillCaps` (AC-2.3.g).
- Test-2.3.h: Regression — the no-recurring-timer grep (AC-2.3.h), a standing fence against regressing to the extension's 60-second poll.
- Test-2.3.i: Unit — the whole ingestor suite (AC-2.3.i).

**Verification Commands**

```bash
set -euo pipefail
F=Packages/macOS/StokdWidgetKit/Sources/StokdWidgetKit/Tokens/TokenUsageIngestor.swift
test -f "$F"
grep -q 'RecursivePathWatcher' "$F"
grep -q 'foldDebounce' "$F"
if grep -nE 'RecursivePathWatcher\(paths:[^)]*\)!' "$F"; then
  echo "FAIL: force-unwrapped failable watcher init" >&2; exit 1
fi
if grep -rnE 'Timer\.scheduledTimer|DispatchQueue.*asyncAfter|RunLoop.*add' "$F"; then
  echo "FAIL: recurring-timer construct in the watch-based ingestor" >&2; exit 1
fi
swift test --package-path Packages/macOS/StokdWidgetKit --filter TokenUsageIngestorTests
```
...
- Test-2.4.a: Unit — `StokdModelRegistryTests.decodesProviderGroups` against a committed fixture of real CLI output (AC-2.4.a).
- Test-2.4.b: Regression — the pricing grep (AC-2.4.b).
- Test-2.4.c: Unit — `StokdModelRegistryTests.normalizesProviderKeysLikeCLI` (AC-2.4.c).
- Test-2.4.d: Regression — `ModelPriceTableTests.substringMatchIsOrderSensitive` (AC-2.4.d). Reordering the table must fail this.
- Test-2.4.e: Unit — `ModelPriceTableTests.unpricedCacheDimensionContributesZero` (AC-2.4.e).
- Test-2.4.f: Unit — `ModelPriceTableTests.unknownModelIsUnpricedNotZero` (AC-2.4.f).
- Test-2.4.g: Integration — the live CLI contract probe (AC-2.4.g).
- Test-2.4.h: Unit — the price-table suite (AC-2.4.h).

**Verification Commands**

```bash
set -euo pipefail
swift test --package-path Packages/macOS/StokdWidgetKit --filter StokdModelRegistryTests
swift test --package-path Packages/macOS/StokdWidgetKit --filter ModelPriceTableTests
if grep -q 'pricing' Packages/macOS/StokdWidgetKit/Sources/StokdWidgetKit/Model/StokdModelRegistry.swift; then
  echo "FAIL: registry reads the always-null pricing field" >&2; exit 1
fi
stokd model list --json | python3 -c 'import json,sys; d=json.load(sys.stdin); assert isinstance(d,list); assert all("provider" in g and "models" in g for g in d); print("models:", sum(len(g["models"]) for g in d))'
```
...
- Test-2.5.a: Regression — the store-reference grep (AC-2.5.a), fencing the `LazyLayoutViewCache` CPU-spin class described in `CLAUDE.md`.
- Test-2.5.b: Unit — `WidgetTileTests.declaredMinimumSizesPerStyle` (AC-2.5.b).
- Test-2.5.c: Regression — appearance-derived color audit (AC-2.5.c).
- Test-2.5.d: Unit — the tile suite (AC-2.5.d).
- Test-2.5.e: Unit — `WidgetTileTests.overflowScrollsWithoutGrowing` (AC-2.5.e).

**Verification Commands**

```bash
set -euo pipefail
F=Packages/macOS/StokdWidgetKit/Sources/StokdWidgetKit/UI/WidgetTile.swift
test -f "$F"
if grep -nE '@(ObservedObject|EnvironmentObject|StateObject|Bindable)' "$F"; then
  echo "FAIL: WidgetTile holds a store property wrapper" >&2; exit 1
fi
if grep -nE '^[[:space:]]*(let|var)[[:space:]]+[a-zA-Z_]+:[[:space:]]*[A-Za-z]*(Store|Manager|Panel)\b' "$F"; then
  echo "FAIL: WidgetTile holds a plain store-typed property" >&2; exit 1
fi
swift test --package-path Packages/macOS/StokdWidgetKit --filter WidgetTileTests
```
...
- Test-2.6.a: Unit — `StokdWidgetRegistrationTests.kindsAreEnumeratedAndRoutable` (AC-2.6.a).
- Test-2.6.b: Regression — the class-presence loop (AC-2.6.b).
- Test-2.6.c: Unit — `StokdWidgetRegistrationTests.openOrFocusIsSingleton` (AC-2.6.c).
- Test-2.6.d: Regression — `StokdWidgetSnapshotTests.payloadRoundTripIncludingLegacyCodec` (AC-2.6.d).
- Test-2.6.e: Unit — `StokdWidgetRegistrationTests.featureGateSuppressesFactoriesAndRestore` (AC-2.6.e).
- Test-2.6.f: Regression — config-key grep (AC-2.6.f).
- Test-2.6.g: Regression — `SurfaceKind` constant grep (AC-2.6.g).
- Test-2.6.h: Regression — pbxproj wiring lint + normalization check (AC-2.6.h). Without the wiring lint, every later test in this PRD could vacuously "pass".
- Test-2.6.i: Integration — the app-host suites execute with a non-zero test count (AC-2.6.i).

**Verification Commands**

```bash
set -euo pipefail
for c in StokdConfigWidgetPanel StokdTokenUsageWidgetPanel; do
  grep -rq "class $c" Sources/Panels/Stokd/ || { echo "FAIL: missing $c" >&2; exit 1; }
done
test -f Sources/Workspace+StokdWidgetPanes.swift
grep -q 'stokdConfigWidget' Packages/macOS/CmuxDockable/Sources/CmuxDockable/DockableKind.swift
grep -q 'stokdTokenUsageWidget' Packages/macOS/CmuxDockable/Sources/CmuxDockable/DockableKind.swift
grep -q 'stokdConfigWidget' Packages/macOS/CmuxWorkspaces/Sources/CmuxWorkspaces/Core/Values/SurfaceKind.swift
grep -q 'stokdTokenUsageWidget' Packages/macOS/CmuxWorkspaces/Sources/CmuxWorkspaces/Core/Values/SurfaceKind.swift
grep -q 'widgets' Sources/CmuxConfig.swift
./scripts/lint-pbxproj-test-wiring.sh
./scripts/check-pbxproj.sh
# Capture output: xcodebuild exits 0 even when it reports "Executed 0 tests".
xcodebuild -project cmux.xcodeproj -scheme cmux-unit -configuration Debug \
  -destination 'platform=macOS' -derivedDataPath /tmp/cmux-stokd-widgets \
  CMUX_SKIP_ZIG_BUILD=1 \
  -only-testing:cmuxTests/StokdWidgetRegistrationTests \
  -only-testing:cmuxTests/StokdWidgetSnapshotTests test 2>&1 | tee /tmp/cmux-stokd-widgets-2.6.log
if grep -q 'Executed 0 tests' /tmp/cmux-stokd-widgets-2.6.log; then
  echo "FAIL: suite executed 0 tests (pbxproj wiring missing)" >&2; exit 1
fi
```
...
**Purpose:** Cannot start before Phase 2 because each widget view needs both a mounted, registered, persistable dockable pane (2.5, 2.6) and its data source (2.3, 2.4) — a view written earlier would have nothing to render into and nothing to render. The two widgets are separate work items in one phase rather than two phases because neither depends on the other.
...
- Test-3.1.a: Unit — `StokdWidgetContextResolverTests.resolvesKeyWindowNotHostWindow` (AC-3.1.a).
- Test-3.1.b: Unit — `StokdWidgetContextResolverTests.focusedSurfaceDirectoryWins` (AC-3.1.b).
- Test-3.1.c: Regression — the merge grep (AC-3.1.c).
- Test-3.1.d: Regression — `StokdWidgetContextResolverTests.plainCurrentDirectoryDidSetUpdatesContext` (AC-3.1.d). This asserts the exact publisher-filter trap.
- Test-3.1.e: Unit — `StokdWidgetContextResolverTests.absentWorkspaceConfigResolvesCreateTarget` (AC-3.1.e).
- Test-3.1.f: Regression — the body-mutation grep (AC-3.1.f).
- Test-3.1.g: Integration — the resolver suite in the app host (AC-3.1.g).

**Verification Commands**

```bash
set -euo pipefail
F=Sources/Panels/Stokd/StokdWidgetContextResolver.swift
test -f "$F"
grep -q 'MergeMany' "$F"
grep -q 'currentDirectory' "$F"
grep -q 'didBecomeKeyNotification' "$F"
xcodebuild -project cmux.xcodeproj -scheme cmux-unit -configuration Debug \
  -destination 'platform=macOS' -derivedDataPath /tmp/cmux-stokd-widgets \
  CMUX_SKIP_ZIG_BUILD=1 \
  -only-testing:cmuxTests/StokdWidgetContextResolverTests test
```
...
- Test-3.2.a: Unit — `StokdConfigWidgetTests.everyDescriptorTypeHasAControl`, driven by the live-captured descriptor fixture so a new CLI type fails loudly (AC-3.2.a).
- Test-3.2.b: Unit — `StokdConfigWidgetTests.provenanceBadgeMatchesOrigin` (AC-3.2.b).
- Test-3.2.c: Security — `StokdConfigWidgetTests.globalWriteRequiresConfirmation` (AC-3.2.c). Guards the agent-off-limits global config.
- Test-3.2.d: Unit — `StokdConfigWidgetTests.globalScopedFieldsDisabledInWorkspaceScope` (AC-3.2.d).
- Test-3.2.e: Regression — `StokdConfigWidgetTests.failedWritePreservesEditAndShowsStderr` (AC-3.2.e).
- Test-3.2.f: Unit — `StokdConfigWidgetTests.successfulWriteReReadsLayer` (AC-3.2.f).
- Test-3.2.g: Regression — the snapshot-boundary grep (AC-3.2.g).
- Test-3.2.h: Integration — the widget suite in the app host (AC-3.2.h).

**Verification Commands**

```bash
set -euo pipefail
F=Sources/Panels/Stokd/StokdConfigWidgetView.swift
test -f "$F"
grep -q 'WidgetTile' "$F"
if grep -nE '@(ObservedObject|EnvironmentObject|StateObject|Bindable)' "$F"; then
  echo "FAIL: config widget holds a store property wrapper" >&2; exit 1
fi
if grep -nE '^[[:space:]]*(let|var)[[:space:]]+[a-zA-Z_]+:[[:space:]]*[A-Za-z]*(Store|Manager|Panel)\b' "$F"; then
  echo "FAIL: config widget row holds a plain store-typed property" >&2; exit 1
fi
xcodebuild -project cmux.xcodeproj -scheme cmux-unit -configuration Debug \
  -destination 'platform=macOS' -derivedDataPath /tmp/cmux-stokd-widgets \
  CMUX_SKIP_ZIG_BUILD=1 \
  -only-testing:cmuxTests/StokdConfigWidgetTests test
```
...
- Test-3.3.a: Unit — `StokdTokenUsageWidgetTests.configuredProvidersAlwaysAppearWithState` (AC-3.3.a).
- Test-3.3.b: Unit — `StokdTokenUsageWidgetTests.grokCacheDimensionsUnavailable` (AC-3.3.b).
- Test-3.3.c: Regression — `StokdTokenUsageWidgetTests.unpricedModelsExcludedFromTotal` (AC-3.3.c). Guards the fabricated-zero-cost failure.
- Test-3.3.d: Unit — `StokdTokenUsageWidgetTests.unknownBucketIsVisible` (AC-3.3.d).
- Test-3.3.e: Performance — `StokdTokenUsageWidgetTests.timespanSwitchPerformsNoIO` (AC-3.3.e).
- Test-3.3.f: Regression — the no-timer grep (AC-3.3.f).
- Test-3.3.g: Regression — the snapshot-boundary grep (AC-3.3.g).
- Test-3.3.h: Integration — the widget suite in the app host (AC-3.3.h).

**Verification Commands**

```bash
set -euo pipefail
F=Sources/Panels/Stokd/StokdTokenUsageWidgetView.swift
test -f "$F"
grep -q 'WidgetTile' "$F"
for pat in 'Timer\.scheduledTimer' 'DispatchQueue.*asyncAfter' '@ObservedObject' '@EnvironmentObject' '@StateObject' '@Bindable'; do
  if grep -nE "$pat" "$F"; then echo "FAIL: $pat in token usage widget" >&2; exit 1; fi
done
if grep -nE '^[[:space:]]*(let|var)[[:space:]]+[a-zA-Z_]+:[[:space:]]*[A-Za-z]*(Store|Manager|Panel)\b' "$F"; then
  echo "FAIL: token usage widget row holds a plain store-typed property" >&2; exit 1
fi
xcodebuild -project cmux.xcodeproj -scheme cmux-unit -configuration Debug \
  -destination 'platform=macOS' -derivedDataPath /tmp/cmux-stokd-widgets \
  CMUX_SKIP_ZIG_BUILD=1 \
  -only-testing:cmuxTests/StokdTokenUsageWidgetTests test
```
...
**Purpose:** Cannot start before Phase 3 because there is nothing to open, localize, or enable until both widget views exist and render. This phase also flips the feature gate default, which must be the last change — enabling a half-built widget would ship a broken surface.
...
- Test-4.1.a: Regression — `StokdWidgetEntrypointTests.allEntrypointsShareOneActionPath` (AC-4.1.a).
- Test-4.1.b: Integration — `StokdWidgetEntrypointTests.paletteCommandsOpenSingleton` (AC-4.1.b).
- Test-4.1.c: Unit — `StokdWidgetEntrypointTests.shortcutsRegisteredAndEditable` (AC-4.1.c).
- Test-4.1.d: Integration — `StokdWidgetEntrypointTests.socketVerbsDispatch` (AC-4.1.d).
- Test-4.1.e: Regression — docs grep (AC-4.1.e).
- Test-4.1.f: Security — `StokdWidgetEntrypointTests.gateOffBlocksEveryEntrypoint` (AC-4.1.f).
- Test-4.1.g: Integration — the entrypoint suite (AC-4.1.g).

**Verification Commands**

```bash
set -euo pipefail
test -f Sources/StokdWidgetActions.swift
test -f Sources/ContentView+StokdWidgetCommandPalette.swift
test -f Sources/TerminalController+ControlStokdWidgets.swift
grep -q 'palette.openStokdConfigWidget' Sources/ContentView+StokdWidgetCommandPalette.swift
grep -q 'palette.openStokdTokenUsageWidget' Sources/ContentView+StokdWidgetCommandPalette.swift
grep -q 'workspace.stokd.configWidget.open' Sources/TerminalController+ControlStokdWidgets.swift
grep -q 'workspace.stokd.tokenUsage.open' Sources/TerminalController+ControlStokdWidgets.swift
grep -q 'stokd.widgets.enabled' docs/configuration.md
xcodebuild -project cmux.xcodeproj -scheme cmux-unit -configuration Debug \
  -destination 'platform=macOS' -derivedDataPath /tmp/cmux-stokd-widgets \
  CMUX_SKIP_ZIG_BUILD=1 \
  -only-testing:cmuxTests/StokdWidgetEntrypointTests test
```
...
- Test-4.2.a: Unit — `check-localization-coverage.py` reports zero gaps for `stokdWidget.*` across all 20 locales (AC-4.2.a).
- Test-4.2.b: Integration — the coverage script runs clean as invoked in CI (AC-4.2.b).
- Test-4.2.c: Regression — `tests/test_ci_localization_coverage.py`, a meta-test asserting the script exits non-zero on a synthetic single-locale gap (AC-4.2.c). Without this the gate itself could be vacuous.
- Test-4.2.d: Regression — CI wiring grep (AC-4.2.d).
- Test-4.2.e: Regression — the bare-literal sweep over the files added by Phase 3 and 4.1 (AC-4.2.e).
- Test-4.2.f: Regression — the catalog key probe (AC-4.2.f).

**Verification Commands**

```bash
set -euo pipefail
test -f scripts/check-localization-coverage.py
python3 scripts/check-localization-coverage.py --prefix stokdWidget
python3 tests/test_ci_localization_coverage.py
grep -q 'check-localization-coverage' .github/workflows/ci.yml
python3 -c "import json; d=json.load(open('Resources/Localizable.xcstrings')); ks=[k for k in d['strings'] if k.startswith('stokdWidget.')]; assert ks, 'no stokdWidget keys'; print('localized keys:', len(ks))"
```

### 4.3 Upstream PR submission, gate default, and final green

**Landing:** upstream-PR (the 2.2 unit) + fork-only (everything else)
**Dependencies:** `4.2`
...
- Test-4.3.a: Security — the upstream-diff purity check (AC-4.3.a), enforcing the fork/upstream boundary.
- Test-4.3.b: Unit — `CmuxConfigTests.stokdWidgetsEnabledDefaultsTrue` (AC-4.3.b).
- Test-4.3.c: Regression — changelog grep (AC-4.3.c).
- Test-4.3.d: Regression — fork-doc grep (AC-4.3.d).
- Test-4.3.e: Regression — the full workflow-guard sweep (AC-4.3.e).
- Test-4.3.f: Unit — both package suites (AC-4.3.f).
- Test-4.3.g: Integration — the full app-host suite (AC-4.3.g).
- Test-4.3.h: E2E — the tagged Debug build (AC-4.3.h).
...
python3 scripts/check-localization-coverage.py --prefix stokdWidget
swift test --package-path Packages/macOS/StokdWidgetKit
swift test --package-path Packages/macOS/CmuxFoundation --filter JSONLTailReaderTests
grep -q 'widgets' Sources/CmuxConfig.swift
grep -qi 'token usage' CHANGELOG.md
grep -qi 'stokd' docs/ghostty-fork.md
# Prepare and verify the upstream-bound branch in a throwaway worktree, so the
# purity check runs against real state rather than assuming a branch exists.
git fetch origin upstream-main -q
git worktree add /tmp/cmux-upstream-jsonl -b upstream/jsonl-tail-reader origin/upstream-main -q
git -C /tmp/cmux-upstream-jsonl cherry-pick "$(git log --format=%H -1 --grep='JSONLTailReader')"
CHANGED=$(git -C /tmp/cmux-upstream-jsonl diff origin/upstream-main --name-only)
echo "$CHANGED" | grep -q 'FileWatch/JSONLTailReader.swift'
test "$(echo "$CHANGED" | grep -c 'JSONLTailReader')" = "$(echo "$CHANGED" | wc -l | tr -d ' ')"
test "$(git -C /tmp/cmux-upstream-jsonl diff origin/upstream-main | grep -ci stokd)" = "0"
git worktree remove /tmp/cmux-upstream-jsonl --force
xcodebuild -project cmux.xcodeproj -scheme cmux-unit -configuration Debug \
  -destination 'platform=macOS' -derivedDataPath /tmp/cmux-stokd-widgets \
  CMUX_SKIP_ZIG_BUILD=1 \
  -only-testing:cmuxTests/StokdWidgetRegistrationTests \
  -only-testing:cmuxTests/StokdWidgetSnapshotTests \
  -only-testing:cmuxTests/StokdWidgetContextResolverTests \
  -only-testing:cmuxTests/StokdConfigWidgetTests \
  -only-testing:cmuxTests/StokdTokenUsageWidgetTests \
  -only-testing:cmuxTests/StokdWidgetEntrypointTests test
CMUX_SKIP_ZIG_BUILD=1 ./scripts/reload.sh --tag stokd-widgets
```
...
- The Dockable gate held: `DockableKind`, `Dockable`, and `DockableSnapshot` exist in `main` and both new kinds are registered, enumerated, and routable.
- `Packages/macOS/StokdWidgetKit` exists, builds, is registered in the workspace, and is listed in the CI `PACKAGES` array so its tests are a real gate.
- Both widgets open as singleton dockable canvas panes from the command palette, a keyboard shortcut, a socket/CLI verb, and a context menu — all through one shared action path — and survive an app restart with their payload state intact.
- The Global Config widget renders all 74 descriptor fields grouped by their 23 groups, shows per-layer provenance, and writes exclusively through the canonical CLI writer with `--workspace` as the default scope and an explicit confirmation for global writes.
- The config widget's scope follows the active window's focused-workspace cwd live, including the case where no workspace config file exists yet.
- The Token Usage widget shows provider → model breakdowns with all four token dimensions and cost across `24h` / `week` / `month` / `total`, driven by a file watch with zero polling timers, with unpriced models excluded from cost totals and unattributed usage visible as `unknown`.
- Every configured provider appears in one of the three explicit states, and no configured provider is silently omitted.
- All `stokdWidget.*` strings are populated for all 20 locales, and `scripts/check-localization-coverage.py` is wired into CI and proven to fail on a gap.
- The single upstream-bound unit (`JSONLTailReader`) is prepared as a stokd-free PR branched from `origin/upstream-main`.
- `stokd.widgets.enabled` defaults to `true`, and every gate in AC-4.3.e/f/g/h passes.

## 4. Rollout & Validation

### Rollout Strategy

- **Gated from the first commit.** `stokd.widgets.enabled` defaults `false` through Phases 1–4.2; only 4.3 flips it. Every entrypoint and session-restore path checks the gate, so a partially-built widget is unreachable rather than broken.
- **Phase-sequential landing.** Each phase lands as its own PR whose Verification Commands all exit 0 before the next begins. Phase 2's ingest work and widget-host work may land as two independent PRs within the phase.
- **Two-commit structure for every regression test**, per the repo policy: commit 1 adds the failing test (CI red), commit 2 adds the fix (CI green). This applies in particular to Test-2.1.b (Claude double-count), Test-2.2.b (partial-line delivery), Test-2.3.f (no-polling fence), Test-2.4.d (price-table ordering), Test-3.1.d (the `presentedDirectoryOnly` publisher trap), and Test-3.3.c (unpriced cost exclusion).
- **Dogfood after each of Phase 3 and Phase 4** on a tagged build (`./scripts/reload.sh --tag stokd-widgets`), never on an untagged `cmux DEV.app`.
- **Rollback triggers:** any main-thread hitch attributable to the ingestor; any incorrect cost figure shown as authoritative; a config write landing in the wrong scope; sustained CPU from the watcher at idle. Rollback is flipping `stokd.widgets.enabled` to `false`, which is why every path is gated.

### Post-Launch Validation

- **Resource cost at idle:** with both widgets open and no agent running, the app's CPU share attributable to ingest should be indistinguishable from baseline. Measure with the widgets open vs closed over 10 minutes.
- **Ingest cost under load:** during an active agent turn, confirm the coalescing holds — folds per minute should track the 750 ms throttle, not the append rate.
- **Attribution quality:** track the `unknown` bucket's share of total tokens. A rising share means a provider's transcript format drifted; it should stay near zero for Claude and Codex.
- **Price coverage:** track the count of `unpriced` / `unregistered` models. `PRICING_SOURCE_VERSION` is `2026-04-24` and already stale against current model ids (`claude-fable-5` matches no pattern; `claude-opus-5` matches the `opus` substring at Opus-4 rates), so this count starting non-zero is expected and is the signal for refreshing the table.
- **Config write correctness:** confirm writes land in the file the widget names, for both scopes, and that a validator rejection surfaces the CLI's own message.
- **Context correctness:** move between windows whose workspaces have different repos and confirm the config widget follows the key window every time, including with no workspace config present.
- **Restart fidelity:** both payloads restore, and a session file naming a disabled widget restores as absent rather than crashing.

## 5. Open Questions

Resolved decisions, recorded per the autonomous-decision rule:

- **Decision D1: widget tiles as dockable canvas panes, not WidgetKit** — chose in-app `Dockable` panes styled as compact tiles because WidgetKit runs in a separate process on an OS-throttled timeline budget and can neither observe the host app's active-window cwd nor sustain a live file watch, both of which are explicit requirements. A WidgetKit mirror reading a shared-container snapshot remains possible later and is out of scope here.
- **Decision D2: target the post-Dockable shape and gate on it** — chose to gate Phase 1 on the unlanded `mission/canvas-dockable-refactor-20260725-170934` branch rather than author against the legacy `PanelType` switch, because the legacy shape would mean editing 8 exhaustive switch sites plus a `SessionPanelSnapshot` optional field and then rewriting all of it when the refactor lands. Singleton canvas panes are also exactly the pattern `docs/gdock-side-controls-iteration.md` establishes.
- **Decision D3: FSEvents on transcript roots as the primary source** — chose `RecursivePathWatcher` over the provider transcript directories with byte-offset incremental tailing, because it is the only source that is simultaneously realtime, per-model, and four-dimension. Rejected: 60-second polling of `/api/telemetry/token-usage` (what the extension does — the stated weakness); Mongo change streams (impossible on a standalone `mongod`); the realtime gateway's `telemetry.cost_update` (carries no `model` and no cache dimensions, so it cannot drive a per-model breakdown, and its Redis transport no-ops without `REDIS_URL`).
- **Decision D4: one recursive FSEvents watcher for all roots, not per-file kqueue** — chose a single `RecursivePathWatcher` over `FileWatcher`-per-file, because 4,598 Claude transcripts plus 1,385 Codex transcripts would mean thousands of file descriptors. Its init takes a **plural** `paths: [String]`, so one watcher covers all three roots rather than one per root; that is strictly better than a per-root design and is what 2.3 specifies. Grok's SQLite file gets a separate `FileWatcher` because it is a single path, not a tree.
- **Decision D5: rely on the watcher's built-in coalescing and add the debounce in the ingestor** — `RecursivePathWatcher` exposes **no** `throttle:` parameter; its coalescing is internal and fixed (`streamLatency = 0.25` + `throttleInterval = .milliseconds(250)`, so ~500 ms worst-case change-to-yield). An earlier draft of this PRD specified `throttle: .milliseconds(750)` on the recursive watcher; that is not implementable against the type as it exists, and the criterion was rewritten. The deliberate debounce therefore lives in the ingestor as `foldDebounce` (default 750 ms, clock-injected), which is also the more testable placement. `FileWatcher` *does* accept `throttle:`, so the grok watcher passes it directly. Rejected: adding a `throttle:` parameter to `RecursivePathWatcher` — a reasonable upstream change, but it expands this PRD's upstream surface for no behavioral gain given the built-in coalescing is already sufficient.
- **Decision D6: own cursor file, not stokd's** — chose `Application Support/cmux/stokd-token-cursors.json` over writing to `~/.stokd/telemetry/ingest-cursors.json`, to avoid contending with `stokd telemetry sync` over a shared 828 KB / 3,948-entry file. The record shape is mirrored so the two stay conceptually aligned.
- **Decision D7: `telemetry_sessions` for cold-start backfill only** — chose the REST aggregate as a one-time seed rather than a source of truth, because coverage is 4.5% (25 of 555 docs with non-empty `model_usage`) and stopped sessions TTL out after 7 days. `session_telemetry` is not read at all: 45 bedrock-only rows, different field names, and percent-escaped model keys requiring `decodeModelKey`.
- **Decision D8: never fall back to the input rate for unpriced cache dimensions** — chose to diverge from the Rust implementation (`token_usage.rs:98-105`, which bills unpriced cache dimensions at the input rate) because that silently invents cost. An unpriced dimension contributes 0 and marks the row `.partiallyPriced`.
- **Decision D9: default write scope is workspace** — chose `--workspace` as the default and required explicit confirmation for global writes, because `~/.stokd/config.yaml` reshapes behavior across every repo and session and is agent-off-limits by repo hook.
- **Decision D10: add a localization CI gate** — chose to introduce `scripts/check-localization-coverage.py` rather than rely on review, because this PRD adds strings across 20 locales and no localization gate exists today despite `CLAUDE.md` requiring an audit for every user-facing change.
- **Decision D11: exactly one upstream unit** — chose to upstream only `JSONLTailReader`. Everything else is stokd-coupled and fork-only by nature; splitting more would produce upstream PRs that leak fork concepts.
- **Decision D12: this PRD does not depend on the companion PRD's `StokdKit`** — chose a narrow `StokdWidgetKit` because `StokdKit` does not exist in this repo in any form and the companion PRD is itself gated on the same Dockable refactor. If `StokdKit` later lands, `StokdWidgetKit`'s CLI runner and config decoder are the natural pieces to merge into it.

Genuinely open, and blocking no Phase 1 or 2 work item:

- **Q1: refreshing the price table.** `PRICING_SOURCE_VERSION` is `2026-04-24` and already wrong for current model ids. Should the widget ship a corrected table (diverging from `mono`'s), or should the fix land in `packages/shared/src/types/cost.ts` first and be ported? Until resolved, affected models render `.unpriced` — correct but less useful. Does not block: the resolution mechanism and its honesty markers are specified independent of the table's contents.
- **Q2: `droid`, `amp`, `bedrock`, `openrouter`, `lmStudio`, and `devin` have no local transcript store**, so usage through them is invisible to a watch-based ingest. Should a later phase add a Stop-hook-driven ingest for them, or is "configured but un-instrumented" an acceptable permanent state? Does not block: those providers render in an explicit un-instrumented state from 3.3 onward.
- **Q3: multi-window semantics.** The current design has a widget in a non-key window render the key window's context while labeling which workspace it reports on. The alternative — pinning a widget to its host window's workspace — may be preferable for users who want two config widgets side by side. Does not block Phases 1–2; revisit before 3.1 lands if dogfood suggests pinning.